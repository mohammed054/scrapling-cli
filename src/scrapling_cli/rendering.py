...
from .utils import build_filename, format_duration, format_number, output_date, prune_markdown_files, write_text
...
def _render_tags(tags: list[str]) -> str:
    if not tags:
        return "_No tags._"
    return " ".join(f"`{tag}`" for tag in tags[:40])
...
def _render_chapters(chapters: list[dict]) -> str:
    ...
    return "\n".join(
        f"- `{format_duration(chapter.get('start', 0))}` - {chapter.get('title', '').strip()}"
...
def render_markdown(item: ContentItem, *, include_score_details: bool) -> str:
    ...
    description = item.description.strip() or "_No description provided._"
    thumbnail_block = f"![Thumbnail]({item.thumbnail})\n\n" if item.thumbnail else ""

    sections = [
        thumbnail_block + f"# {item.title}\n",
        "---\n\n## Info\n\n"
        "| Field | Value |\n"
        "|-------|-------|\n"
        f"| **Date** | {date_text} |\n"
        ...
        f"| **Channel** | [{item.channel}]({item.channel_url}) |\n"
