...
from .utils import format_duration, format_number, output_date, repair_text, write_text
...
def _top_table(items: list[ContentItem], *, limit: int = 25) -> str:
    ...
    for index, item in enumerate(items[:limit], 1):
        ...
        rows.append(
            f"| {index} | [{repair_text(item.title)[:55]}]({item.url}) | {date_text} | "
...
def _tags_section(items: list[ContentItem]) -> str:
    ...
    for item in items:
        for tag in item.tags:
            counts[repair_text(tag).lower()] += 1
    if not counts:
...
def _engagement_note(items: list[ContentItem]) -> str:
    if not items:
        return ""
    if all(item.likes == 0 and item.comments == 0 for item in items):
        return (
            "> Engagement note: YouTube did not expose likes/comments for this scored run, "
            "so ranking is effectively view-driven.\n"
        )
    return ""
...
def generate_channel_report(
    *,
    ...
    output_dir: Path,
    scraped_item_count: int | None = None,
    ...
    scored_items = all_videos + all_shorts
    pool_note = ""
    if scraped_item_count is not None and candidate_item_count is not None:
        pool_note = (
            f"> Unique items scraped: **{format_number(scraped_item_count)}**  \n"
            f"> Candidate items scored: **{format_number(candidate_item_count)}**\n\n"
        )
    engagement_note = _engagement_note(scored_items)

    body = f"""# Channel Performance Report: {repair_text(channel_name)}
...
|--------|--------|--------|
| Candidate items scored | {format_number(video_stats['count'])} | {format_number(short_stats['count'])} |
            ...
            writer.writerow(
                {
                    ...
                    "transcript_source": repair_text(item.transcript.source),
                    "transcript_language": repair_text(item.transcript.language),
                    "transcript_chars": item.transcript.characters,
                    "transcript_error": repair_text(item.transcript.error),
                    "tags": "|".join(repair_text(tag) for tag in item.tags),
                    "category": repair_text(item.category),
                    "url": item.url,
                }
            )
