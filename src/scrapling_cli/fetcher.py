...
from .utils import parse_date, slugify_channel_name
    ...
    try:
        ...
        for content in contents:
            ...
            for button in buttons:
                ...
                label = model.get("accessibilityText") or model.get("title", "")
...
def _parse_description(yt_data: dict) -> str:
    try:
        ...
        for content in contents:
            ...
            if secondary:
                return _extract_text_runs(secondary.get("description", {}).get("runs", []))
    except Exception as exc:  # noqa: BLE001
...
def _parse_chapters(player_data: dict) -> list[dict]:
    ...
    try:
        ...
        for block in chapter_blocks:
            ...
            chapters.append(
                {
                    "title": _extract_text_runs(renderer.get("title", {}).get("runs", [])),
...
def enrich_content_item(item: ContentItem, *, retries: int = 2) -> ContentItem:
    ...
    for attempt in range(1, retries + 1):
        ...
        item.description = video_details.get("shortDescription", "") or _parse_description(yt_data)
        item.chapters = _parse_chapters(player_data)
        item.subscribers = _parse_subscribers(yt_data)
        item.channel = item.channel or video_details.get("author", "")
        ...
        item.tags = list(video_details.get("keywords", []))
